...
PWA de **consulta (read-only)** de finanças pessoais do Diogo. Lê o `finance.db` (SQLite, ~3 MB) diretamente do OneDrive do utilizador via Microsoft Graph, abre-o no browser com sql.js e apresenta 6 módulos de análise. Não existe backend nem servidor: React + Vite + TypeScript, alojado em GitHub Pages, instalável como PWA.

A app desktop original (Python/Streamlit) continua a ser a única a **escrever** dados (ETL semanal). Esta app **nunca modifica** o `finance.db`.
...
3. **Philosophy B** — separação do imobiliário (unidades 5L, 7D, 7E):
...
7. **Sem segredos no repo**: apenas o `clientId` do Entra ID (público por natureza em SPAs) pode estar na config. Nunca commitar tokens, API keys ou o próprio `finance.db`.
